...
bool RobotDriver::updateState() {
  if (is_hardware_) {
    ...
    // Pass IMU to the learned policy for acceleration access. Hoisted out of
    // the no-mocap branch below so the policy is fed on every tick regardless
    // of which path fills the state. Guarded because LearnedVelocityPolicy is
    // only declared when the workspace was built with ONNX Runtime; without
    // it, these controller ids would have been rejected earlier in
    // initLegController() so this is a no-op anyway. The cast catches
    // UnderbrushPolicy too (derived).
#ifdef HAS_ONNXRUNTIME
    if (auto c =
            std::dynamic_pointer_cast<LearnedVelocityPolicy>(leg_controller_)) {
    ...
    // Fallback for running the learned controllers WITHOUT motion capture:
    // fake the state from IMU + joint encoders. The learned policy only needs
    // orientation, angular velocity, joint positions/velocities, and (for
    // underbrush) joint effort — all available from onboard sensors. Foot
    // contact is plumbed separately above. With mocap (use_mocap_), fall
    // through to the real state estimator instead — it supplies a true world
    // pose rather than a body pinned to the origin, and it is the only path
    // that consumes last_mocap_msg_.
    if ((controller_id_ == "learned" || controller_id_ == "underbrush_learned" ||
         controller_id_ == "underbrush_v90" ||
         controller_id_ == "underbrush_v92") &&
        !use_mocap_) {
    ...
    // Everything else (and the learned controllers when mocap is on) runs off
