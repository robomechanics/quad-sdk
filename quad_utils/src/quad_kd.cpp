...
QuadKD::QuadKD() {
  ...
  std::vector<double> joint_min_front = {-0.707, -M_PI, 0};
  std::vector<double> joint_min_back = {-0.707, -M_PI, 0};
  std::vector<double> joint_max_front = {0.707, M_PI, M_PI};
  std::vector<double> joint_max_back = {0.707, M_PI, M_PI};
...
void QuadKD::legbaseToFootIKLegbaseFrame(int leg_index,
  ...
  // We assume the toe is always lower than the hip in the hip-knee plane
