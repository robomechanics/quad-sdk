#ifndef QUAD_ESTIMATORS_EKF_ESTIMATOR_H
#define QUAD_ESTIMATORS_EKF_ESTIMATOR_H

#include <quad_estimators/state_estimator.hpp>
...
class EKFEstimator : public StateEstimator {
 public:
  ...
  // Setters used by the node to feed contact / grf information that was
  // subscribed to externally in robot_driver. Kept so updateOnce can use
  // the same internal members without depending on robot_driver.
  void loadGRFMsg(quad_msgs::msg::GRFArray::SharedPtr msg) {
    last_grf_msg_ = msg;
  }
  void loadContactMsg(quad_msgs::msg::ContactMode::SharedPtr msg) {
...
#endif  // QUAD_ESTIMATORS_EKF_ESTIMATOR_H
