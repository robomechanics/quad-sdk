...
RobotDriver::RobotDriver(ros::NodeHandle nh, int argc, char** argv) {
  ...
  body_force_estimate_sub_ = nh_.subscribe(
    body_force_estimate_topic,1,&RobotDriver::bodyForceEstimateCallback, this);
  ...
  if (is_hw_) {
    ...
    mocap_sub_ = nh_.subscribe(mocap_topic,1000,&RobotDriver::mocapCallback, this, ros::TransportHints().tcpNoDelay(true));
    ...
    robot_state_sub_ = nh_.subscribe(robot_state_topic,1,&RobotDriver::robotStateCallback, this, ros::TransportHints().tcpNoDelay(true));
  }
...
void RobotDriver::legOverrideCallback(const quad_msgs::LegOverride::ConstPtr& msg) {
  last_leg_override_msg_ = *msg;
}

void RobotDriver::bodyForceEstimateCallback(const quad_msgs::BodyForceEstimate::ConstPtr& msg) {
  if (UnderbrushInverseDynamicsController* c = dynamic_cast<UnderbrushInverseDynamicsController*>(leg_controller_.get())) {
...
void RobotDriver::checkMessages() {
    ...
    remote_heartbeat_received_time_ != std::numeric_limits<double>::max())
  {
    ...
    ROS_WARN_THROTTLE(1,"Remote heartbeat lost or late to robot driver node, entering safety mode");
  }

  // Check the state message latency
  if (!is_hw_ && abs(ros::Time::now().toSec() - last_state_time_) >= state_timeout_ && 
    last_state_time_ != std::numeric_limits<double>::max())
  {
    control_mode_ = SAFETY;
    transition_timestamp_ = ros::Time::now();
    ROS_WARN_THROTTLE(1,"State messages lost in robot driver node, entering safety mode");
  }
...
bool RobotDriver::updateControl() {
  ...
  { 
    for (int i = 0; i < num_feet_; ++i) {
      ...
      for (int j = 0; j < 3; ++j) {
        ...
        robot_driver_utils::loadMotorCommandMsg(0,0,0,safety_kp_.at(j),safety_kd_.at(j),
  ...
  { 
    for (int i = 0; i < num_feet_; ++i) {
      ...
      for (int j = 0; j < 3; ++j) {
        ...
        robot_driver_utils::loadMotorCommandMsg(sit_joint_angles_.at(j),0,0,
          sit_kp_.at(j), sit_kd_.at(j), leg_command_array_msg_.leg_commands.at(i).motor_commands.at(j));
      }
    }
  } 
  else if (control_mode_ == READY)
  {
    if (leg_controller_->computeLegCommandArray(last_robot_state_msg_,
      ...
      for (int i = 0; i < num_feet_; ++i) {
        ...
        for (int j = 0; j < 3; ++j) {
          int joint_idx = 3*i+j;
          robot_driver_utils::loadMotorCommandMsg(stand_joint_angles_.at(j),0,0,
            stand_kp_.at(j),stand_kd_.at(j), leg_command_array_msg_.leg_commands.at(i).motor_commands.at(j));
        }
      }
    }
  ...
  else if (control_mode_ == SIT_TO_READY)
  {
    ...
    for (int i = 0; i < num_feet_; ++i) {
      leg_command_array_msg_.leg_commands.at(i).motor_commands.resize(3);
      for (int j = 0; j < 3; ++j) {
        double ang = (stand_joint_angles_.at(j) - sit_joint_angles_.at(j))*t_interp + 
          sit_joint_angles_.at(j);

        robot_driver_utils::loadMotorCommandMsg(ang,0,0,stand_kp_.at(j), stand_kd_.at(j),
          leg_command_array_msg_.leg_commands.at(i).motor_commands.at(j));
      }
    }
  }
  else if (control_mode_ == READY_TO_SIT)
  {
    ...
    for (int i = 0; i < num_feet_; ++i) {
      leg_command_array_msg_.leg_commands.at(i).motor_commands.resize(3);
      for (int j = 0; j < 3; ++j) {
        double ang = (sit_joint_angles_.at(j) - stand_joint_angles_.at(j))*t_interp + 
...
void RobotDriver::publishHeartbeat() {
  ...
  // Publish hearbeat
  if ((ros::Time::now() - last_robot_heartbeat_msg_.stamp).toSec() >= 1.0/publish_rate_) {
