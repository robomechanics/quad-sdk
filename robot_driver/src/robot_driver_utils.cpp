...
void loadMotorCommandMsg(double pos_setpoint, double vel_setpoint, double ff,
  ...
  // Compute diagnostics
  // loadMotorCommandMsg(leg_idx, joint_idx, pos_act, vel_act, torque_limits,
  // msg);
}

// void loadMotorCommandMsg(int leg_idx, int joint_idx, double pos_act, double
// vel_act,
...
//   msg.fb_ratio = abs(msg.fb_component)/(abs(msg.fb_component) +
//   abs(msg.torque_ff));

//   if (abs(msg.torque_ff) >= torque_limits[joint_idx]) {
//     ROS_WARN("Leg %d motor %d: ff effort = %5.3f Nm exceeds threshold of
//     %5.3f Nm",
//       leg_idx,joint_idx,msg.torque_ff, torque_limits[joint_idx]);

//   }
//   if (abs(msg.effort) >= torque_limits[joint_idx]) {
//     ROS_WARN("Leg %d motor %d: total effort = %5.3f Nm exceeds threshold of
//     %5.3f Nm",
//       leg_idx,joint_idx,msg.effort, torque_limits[joint_idx]);
//     msg.effort = std::min(std::max(msg.effort, -torque_limits[joint_idx]),
//     torque_limits[joint_idx]);
//   }
// }
