...
ContactDetection::ContactDetection(ros::NodeHandle nh) {
  ...
  joint_encoder_sub_ = nh_.subscribe(
      joint_encoder_topic, 1, &ContactDetection::jointEncoderCallback, this);
  imu_sub_ = nh_.subscribe(imu_topic, 1, &ContactDetection::imuCallback, this);
  contact_mode_pub_ =
      nh_.advertise<quad_msgs::ContactMode>(contact_mode_topic, 1);
}

void ContactDetection::jointEncoderCallback(
