...
TrajectoryPublisher::TrajectoryPublisher(ros::NodeHandle nh) {
  ...
  std::string body_plan_topic, foot_plan_continuous_topic, trajectory_topic,
      trajectory_state_topic, ground_truth_state_topic;
  ...
  nh.param<std::string>("topics/state/trajectory", trajectory_state_topic,
                        "/state/trajectory");
  nh.param<std::string>("topics/trajectory", trajectory_topic, "/trajectory");
  ...
  // Setup subs and pubs
  body_plan_sub_ =
      nh_.subscribe(body_plan_topic, 1, &TrajectoryPublisher::robotPlanCallback,
                    this, ros::TransportHints().tcpNoDelay(true));
  multi_foot_plan_continuous_sub_ =
      nh_.subscribe(foot_plan_continuous_topic, 1,
                    &TrajectoryPublisher::multiFootPlanContinuousCallback, this,
                    ros::TransportHints().tcpNoDelay(true));
  ground_truth_state_sub_ = nh_.subscribe(
      ground_truth_state_topic, 1, &TrajectoryPublisher::robotStateCallback,
      this, ros::TransportHints().tcpNoDelay(true));

  trajectory_state_pub_ =
      nh_.advertise<quad_msgs::RobotState>(trajectory_state_topic, 1);
  trajectory_pub_ =
      nh_.advertise<quad_msgs::RobotStateTrajectory>(trajectory_topic, 1);
}
...
void TrajectoryPublisher::updateTrajectory() {
  ...
  if (body_plan_msg_.states.empty() ||
      multi_foot_plan_continuous_msg_.states.empty())
    return;
  ...
  ros::Duration t_foot_plan_ros =
      multi_foot_plan_continuous_msg_.states.back().header.stamp -
  ...
  for (int i = 0; i < t_traj_.size(); i++) {
    ...
    quad_utils::interpRobotPlan(body_plan_msg_, t_traj_[i], state, primitive_id,
                                grf_array);
    ...
    if (t_traj_[i] < t_foot_plan) {
      ...
      quad_utils::fkRobotState(kinematics, state.body, state.joints,
                               state.feet);
    }
...
void TrajectoryPublisher::publishTrajectoryState() {
  ...
  quad_msgs::RobotState interp_state =
      quad_utils::interpRobotStateTraj(traj_msg_, t);
  // quad_msgs::RobotState interp_state =
  // quad_utils::interpRobotStateTraj(traj_msg_,t_mod);
