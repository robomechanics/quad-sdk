...
ContactStatePublisher::ContactStatePublisher()
    : rclcpp::Node("contact_state_publisher"), tf_buffer_(this->get_clock()),listener_(buffer_) {
  nh_ = nh;
...
//       ros::VoidConstPtr(), ros::TransportHints().tcpNoDelay());
  ...
  // Setup pubs
  grf_pub_ = this->create_publisher<quad_msgs::msg::GRFArray>(grf_topic, 10);
