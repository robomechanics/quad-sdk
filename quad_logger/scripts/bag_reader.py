...
ef read_bag(bagfile):
    ...
    # Extract topic types and store them
    topics_and_types = reader.get_all_topics_and_types()
    for topic_info in topics_and_types:
        topic_type_dict[topic_info.name] = topic_info.type
    return topic_type_dict

def read_topic_type():
    """Return the current topic-type dictionary."""
    return topic_type_dict

def read_msg(bagfile, topics):
    """Read messages from specified topics in the bag."""
    storage_options = rosbag2_py.StorageOptions(uri=bagfile, storage_id='sqlite3')
    converter_options = rosbag2_py.ConverterOptions(input_serialization_format='cdr', output_serialization_format='cdr')
    reader = rosbag2_py.SequentialReader()
    reader.open(storage_options, converter_options)
