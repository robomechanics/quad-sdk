...
class RobotDriver {
 ...
 private:
  ...
  /// Whether motion capture is available. With mocap the learned
  /// controllers run off the full state estimator like every other
  /// controller; without it, state is faked from IMU + leg kinematics
  /// (see updateState).
  bool use_mocap_;
