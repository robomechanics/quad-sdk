...
ContactStatePublisher::ContactStatePublisher(ros::NodeHandle nh) : listener_(buffer_) {
  ...
  spirit_utils::loadROSParam(nh_,"topics/gazebo/toe0_contact_state",toe0_contact_state_topic);
  spirit_utils::loadROSParam(nh_,"topics/gazebo/toe1_contact_state",toe1_contact_state_topic);
  spirit_utils::loadROSParam(nh_,"topics/gazebo/toe2_contact_state",toe2_contact_state_topic);
  spirit_utils::loadROSParam(nh_,"topics/gazebo/toe3_contact_state",toe3_contact_state_topic);

  spirit_utils::loadROSParam(nh_,"contact_state_publisher/update_rate", update_rate_);

  // Setup pubs and subs
  toe0_contact_state_sub = nh_.subscribe<gazebo_msgs::ContactsState>(toe0_contact_state_topic,1,
    boost::bind( &ContactStatePublisher::contactStateCallback, this, _1, 0));
  toe1_contact_state_sub = nh_.subscribe<gazebo_msgs::ContactsState>(toe1_contact_state_topic,1,
    boost::bind( &ContactStatePublisher::contactStateCallback, this, _1, 1));
  toe2_contact_state_sub = nh_.subscribe<gazebo_msgs::ContactsState>(toe2_contact_state_topic,1,
    boost::bind( &ContactStatePublisher::contactStateCallback, this, _1, 2));
  toe3_contact_state_sub = nh_.subscribe<gazebo_msgs::ContactsState>(toe3_contact_state_topic,1,
