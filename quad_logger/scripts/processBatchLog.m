...
bagList = dir(strcat(bagPath, '*.bag'));
bAnimate = false;
bSave = false;
...
% Loop all bags
for i = 1:size(bagList, 1)
        
    % Load bag files
    trialName = strcat(bagPath, bagList(i).name);
    ...
    if isempty(data.stateGroundTruth.tailJointPosition)
        maxError(end+1, 1) = max(abs(data.stateGroundTruth.axisAngleRP(:, 4)));
    else
        maxError(end+1, 2) = max(abs(data.stateGroundTruth.axisAngleRP(:, 4)));
    end
