...
MPCController::MPCController(ros::NodeHandle nh) {
  ...
  quad_utils::loadROSParam(nh, "mpc_controller/control_weights",
                           control_weights);
  quad_utils::loadROSParam(nh, "mpc_controller/state_lower_bound",
                           state_lower_bound);
  quad_utils::loadROSParam(nh, "mpc_controller/state_upper_bound",
                           state_upper_bound);
  ...
  // Setup pubs and subs
  robot_state_traj_sub_ = nh_.subscribe(
      robot_state_traj_topic, 1, &MPCController::robotPlanCallback, this);
  robot_state_sub_ = nh_.subscribe(robot_state_topic, 1,
                                   &MPCController::robotStateCallback, this);
...
void MPCController::robotPlanCallback(
    const quad_msgs::RobotStateTrajectory::ConstPtr &msg) {
  last_plan_msg_ = msg;
}

void MPCController::robotStateCallback(
    const quad_msgs::RobotState::ConstPtr &msg) {
  cur_state_ = this->state_to_eigen(*msg);
}

Eigen::VectorXd MPCController::state_to_eigen(quad_msgs::RobotState robot_state,
                                              bool zero_vel) {
...
void MPCController::extractMPCTrajectory(
    int start_idx, std::vector<std::vector<bool>> &contact_sequences,
    Eigen::MatrixXd &foot_positions, Eigen::MatrixXd &ref_traj) {
  ...
  for (int i = 0; i < N_; ++i) {
    ...
    for (int leg_idx = 0; leg_idx < num_legs_; ++leg_idx) {
      contact_sequences.at(i).at(leg_idx) =
          robot_state.feet.feet.at(leg_idx).contact;

      Eigen::Vector3d joint_pos;
      for (int joint_idx = 0; joint_idx < num_joints_per_leg_; ++joint_idx) {
        joint_pos(joint_idx) =
            joint_states.at(leg_idx * num_joints_per_leg_ + joint_idx);
      }
    ...
    // Load state into reference trajectory (w/ zero velocity if we're at end of
    // plan)
...
void MPCController::publishGRFArray() {
  if (last_plan_msg_ == NULL) {
    ROS_WARN_THROTTLE(0.5,
                      "No robot trajectory plan in MPCController, exiting");
    return;
  }
  ...
  if (last_plan_msg_->states.size() == 0) {
    ROS_WARN_THROTTLE(
        0.5,
        "Received robot trajectory with no states in MPCController, exiting");
    return;
  }

  double last_plan_age_seconds =
      quad_utils::getROSMessageAgeInMs(last_plan_msg_->header) / 1000.0;
  ...
  this->extractMPCTrajectory(start_idx, contact_sequences, foot_positions,
                             ref_traj);
  ...
  if (!quad_mpc_->solve(cur_state_, ref_traj, x_out)) {
    ...
    std::cout << "Current state: " << std::endl
              << cur_state_.format(CleanFmt) << std::endl;
    std::cout << "Reference trajectory: " << std::endl
              << ref_traj.format(CleanFmt) << std::endl;
    std::cout << "Foot Placements in body frame: " << std::endl
              << foot_positions.format(CleanFmt) << std::endl;
    throw 10;
  }
