...
GlobalFootstepPlanner::GlobalFootstepPlanner(ros::NodeHandle nh) {
  ...
  nh.param<std::string>("topics/terrain_map", terrain_map_topic_,
                        "/terrain_map");
  nh.param<std::string>("topics/global_plan", body_plan_topic_, "/body_plan");
  nh.param<std::string>("topics/foot_plan_discrete", foot_plan_discrete_topic,
                        "/foot_plan_discrete");
  ...
  terrain_map_sub_ = nh_.subscribe(
      terrain_map_topic_, 1, &GlobalFootstepPlanner::terrainMapCallback, this);
  body_plan_sub_ = nh_.subscribe(
      body_plan_topic_, 1, &GlobalFootstepPlanner::robotPlanCallback, this);
  robot_state_sub_ = nh_.subscribe(
      robot_state_topic, 1, &GlobalFootstepPlanner::robotStateCallback, this);
...
void GlobalFootstepPlanner::robotPlanCallback(
  ...
  if (body_plan_msg_ != NULL) {
    if (msg->states.back().header.stamp ==
        body_plan_msg_->states.back().header.stamp) {
      return;
    }
  }
...
void GlobalFootstepPlanner::robotStateCallback(
  ...
  if (msg->feet.feet.empty()) return;

  if (body_plan_msg_ == NULL) return;
...
void GlobalFootstepPlanner::updateDiscretePlan() {
  ...
  double t_s[num_feet_] = {0.5 * period_, 0.5 * period_, 0.5 * period_,
                           0.5 * period_};
  ...
  for (int j = 0; j < num_feet_; j++) {
    ...
    // If we have robot state data, apply the current foot position for the
    // first cycle
    if (robot_state_msg_ != NULL) {
    ...
    for (int i = start_index; i < end_index; i++) {
      ...
      quad_utils::interpRobotPlan((*body_plan_msg_), t_touchdown,
                                  state_touchdown, primitive_id_touchdown,
                                  grf_array_touchdown);
      quad_utils::interpRobotPlan((*body_plan_msg_), t_midstance,
                                  state_midstance, primitive_id_midstance,
                                  grf_array_midstance);
      ...
      Eigen::Vector3d body_pos_touchdown = {
          state_touchdown.body.pose.position.x,
          state_touchdown.body.pose.position.y,
          state_touchdown.body.pose.position.z};

      Eigen::Vector3d body_vel_midstance = {
          state_midstance.body.twist.linear.x,
      ...
      kinematics.worldToNominalHipFKWorldFrame(j, body_pos_touchdown,
                                               body_rpy_touchdown,
                                               nominal_footstep_pos_touchdown);

      Eigen::Vector3d grf_midstance = {
          grf_array_midstance.vectors[0].x,
      ...
      //   z_hip_midstance};
      Eigen::Vector3d nominal_footstep_pos_midstance =
          nominal_footstep_pos_touchdown + 0.5 * t_s[j] * body_vel_midstance;
      Eigen::Vector3d footstep_grf = terrain_.projectToMap(
          nominal_footstep_pos_midstance, -1.0 * grf_midstance);
      ...
      Eigen::Vector3d footstep_nom =
          (1 - grf_weight_) * nominal_footstep_pos_midstance +
    ...
    if (t_cycle_end >= t_plan_.back()) {
      ...
      kinematics.worldToNominalHipFKWorldFrame(
          j, body_pos_final, body_rpy_final, nominal_footstep_pos_final);
...
void GlobalFootstepPlanner::updateContinuousPlan() {
  ...
  if (footstep_plan_.empty()) {
    ROS_WARN_THROTTLE(0.5,
                      "Footstep plan is empty, not updating or publishing"
                      " swing leg plan");
    return;
  }
...
void GlobalFootstepPlanner::publishDiscretePlan() {
  ...
  for (int i = 0; i < footstep_plan_.size(); ++i) {
    ...
    for (int j = 0; j < footstep_plan_[i].size(); ++j) {
      ...
      foot_state_msg.header.stamp =
          plan_timestamp_ + ros::Duration(footstep_plan_[i][j][2]);
...
void GlobalFootstepPlanner::waitForData() {
  ...
  while ((shared_body_plan == nullptr) && ros::ok()) {
    shared_body_plan =
        ros::topic::waitForMessage<quad_msgs::RobotPlan>(body_plan_topic_, nh_);
    ros::spinOnce();
  }
}
