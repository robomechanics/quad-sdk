...
void MeshToGridMapConverter::subscribeToTopics() {
  mesh_sub_ = nh_.subscribe("mesh", 10, &MeshToGridMapConverter::meshCallback,
...
bool MeshToGridMapConverter::saveGridMapService(
    grid_map_msgs::ProcessFile::Request& request,
...
bool MeshToGridMapConverter::saveGridMap(const grid_map::GridMap& map,
  ...
  if (!path_to_file.empty()) {
    ...
    ROS_ERROR("No rosbag filepath specified where to save grid map.");
    return false;
  }
  return true;
}
