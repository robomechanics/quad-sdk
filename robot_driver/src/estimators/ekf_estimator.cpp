...
//   X_pre = Eigen::VectorXd::Zero(num_state);
//   X_pre.segment(0, 3) = r + dt * v + dt * dt * 0.5 * (C1.transpose() * a +
//   g); X_pre.segment(3, 3) = v + dt * (C1.transpose() * a + g);
//   // quaternion updates
...
//       Q.block<3, 3>(9 + i * 3, 9 + i * 3) = 1* dt * C.transpose() *
//       noise_feet * C; num_contacts++;
//     } else {
