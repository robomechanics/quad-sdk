...
std::string getParameterPath() {
  std::string filePath =
      ros::package::getPath("grid_map_pcl") + "/config/parameters.yaml";
  return filePath;
}
...
std::string getOutputBagPath(const ros::NodeHandle &nh) {
  ...
  nh.param<std::string>("output_grid_map", outputRosbagName,
                        "output_grid_map.bag");
...
void saveGridMap(const grid_map::GridMap &gridMap, const ros::NodeHandle &nh,
  ...
  const bool savingSuccessful = grid_map::GridMapRosConverter::saveToBag(
      gridMap, pathToOutputBag, mapTopic);
  ROS_INFO_STREAM("Saving grid map successful: " << std::boolalpha
                                                 << savingSuccessful);
}
