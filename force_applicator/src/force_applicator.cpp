...
ForceApplicator::ForceApplicator(rclcpp::Node::SharedPtr node) : node_(node) {
  std::string robot_state_topic, force_marker_topic, wrench_persist_topic,
      wrench_clear_topic;
  ...
  quad_utils::loadROSParam(node_, "force_applicator.duration.update_rate",
                           update_rate_);
  quad_utils::loadROSParam(node_, "force_applicator.random.force_mag_min",
                           force_mag_min_);
  quad_utils::loadROSParam(node_, "force_applicator.random.force_mag_max",
                           force_mag_max_);
  quad_utils::loadROSParam(node_, "force_applicator.random.torque_mag_min",
                           torque_mag_min_);
  quad_utils::loadROSParam(node_, "force_applicator.random.torque_mag_max",
                           torque_mag_max_);
  quad_utils::loadROSParam(node_, "force_applicator.periodic.period", period_);
  quad_utils::loadROSParam(node_,
                           "force_applicator.distance.distance_threshold",
                           distance_threshold_);
  ...
  quad_utils::loadROSParam(
      node_, "force_applicator.topics.visualization.force_torque_markers",
      force_marker_topic);
  quad_utils::loadROSParam(node_,
                           "force_applicator.topics.wrench.wrench_persist",
                           wrench_persist_topic);
  ...
  robot_state_sub_ = node_->create_subscription<quad_msgs::msg::RobotState>(
      ...
      std::bind(&ForceApplicator::robotStateCallback, this,
                std::placeholders::_1));
  force_marker_pub_ = node_->create_publisher<visualization_msgs::msg::Marker>(
      force_marker_topic, 10);
  wrench_persist_pub_ =
      node_->create_publisher<ros_gz_interfaces::msg::EntityWrench>(
          wrench_persist_topic, 10);
...
void ForceApplicator::robotStateCallback(
    const quad_msgs::msg::RobotState::SharedPtr msg) {
  last_robot_state_msg_ = *msg;
  have_pose_ = true;
}

double ForceApplicator::computeEuclideanDistance(
...
bool ForceApplicator::shouldTrigger() {
  if (mode_ == "periodic") {
    ...
    if (computeEuclideanDistance(last_robot_pose_, curr_robot_pose) >
        distance_threshold_) {
...
void ForceApplicator::applyForce() {
  ...
  if (force_mode_ == "random") {
    ...
    fz = f.z();
    fz = -std::abs(fz);
    std::uniform_real_distribution<double> t_mag_distribution(torque_mag_min_,
                                                              torque_mag_max_);
    tx = t_mag_distribution(rng_);
  ...
  clear_timer_ = rclcpp::create_timer(
      node_, node_->get_clock(), rclcpp::Duration::from_seconds(dt_), [this]() {
