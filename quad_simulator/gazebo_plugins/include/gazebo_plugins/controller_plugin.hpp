...
//   std::vector<hardware_interface::JointHandle> joints_;
  ...
  void enforceJointLimits(double& command, unsigned int index);
    ...
    // InterfaceType &get_interface(std::vector<InterfaceType> &interfaces, const std::string &name, const std::string &type) {
    // for (auto &interface : interfaces) {
    //     const std::string 
    //     if (interface.get_name() == name && interface.get_interface_name() == type) {
    //     return interface;
    //     }
    // }
    ...
    for (auto &interface : interfaces)
    {
        ...
        const std::string &iface_type = interface.get_interface_name();  // e.g., "effort"

        // Match against fully-qualified name
        if (full_name == joint_name + "/" + interface_type && iface_type == interface_type)
        {
        return interface;
        }
    }

    throw std::runtime_error("Interface not found: " + joint_name + "/" + interface_type);
    }
