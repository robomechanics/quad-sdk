...
OpenLoopController::OpenLoopController(ros::NodeHandle nh) {
  ...
  quad_utils::loadROSParam(nh_, "open_loop_controller/update_rate",
                           update_rate_);
  quad_utils::loadROSParam(nh_, "open_loop_controller/stand_angles",
                           stand_joint_angles_);
  ...
  quad_utils::loadROSParam(nh_, "open_loop_controller/use_diff_for_velocity",
  ...
  control_mode_sub_ = nh_.subscribe(
      control_mode_topic, 1, &OpenLoopController::controlModeCallback, this);
}

void OpenLoopController::controlModeCallback(
  ...
  switch (control_mode_) {
    ...
    case 2:  // walk
    {
      for (int i = 0; i < 4; ++i) {
        ...
        // Find target velocity from central difference of precomputed
        // trajectory
        int prev_idx =
        ...
        double hip_vel = (next_hip_knee_angs.first - prev_hip_knee_angs.first) /
                         (2 * interp_dt_);
        double knee_vel =
            (next_hip_knee_angs.second - prev_hip_knee_angs.second) /
            (2 * interp_dt_);
        ...
        msg.leg_commands.at(i).motor_commands.at(1).pos_setpoint =
            hip_knee_angs.first;
        msg.leg_commands.at(i).motor_commands.at(1).kp = walk_kp_.at(1);
        msg.leg_commands.at(i).motor_commands.at(1).kd = walk_kd_.at(1);
        msg.leg_commands.at(i).motor_commands.at(1).vel_setpoint =
            use_diff_for_velocity_ ? hip_vel : 0;
        msg.leg_commands.at(i).motor_commands.at(1).torque_ff = 0;

        msg.leg_commands.at(i).motor_commands.at(2).pos_setpoint =
            hip_knee_angs.second;
        msg.leg_commands.at(i).motor_commands.at(2).kp = walk_kp_.at(2);
        msg.leg_commands.at(i).motor_commands.at(2).kd = walk_kd_.at(2);
        msg.leg_commands.at(i).motor_commands.at(2).vel_setpoint =
            use_diff_for_velocity_ ? knee_vel : 0;
