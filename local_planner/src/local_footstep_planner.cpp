...
void LocalFootstepPlanner::computeFootPlan(int current_plan_index, const std::vector<std::vector<bool>> &contact_schedule,
  const Eigen::MatrixXd &body_plan, const Eigen::MatrixXd &grf_plan, const Eigen::MatrixXd &ref_body_plan, const Eigen::VectorXi &ref_primitive_plan_, 
  ...
  for (int j=0; j<num_feet_; j++) {
    ...
    for (int i = 0; i < contact_schedule.size(); i++) {
      ...
      if (!isContact(contact_schedule, i, j) || (i != 0 && isNewContact(contact_schedule, i, j)))
      {
        ...
        if (isNewLiftoff(contact_schedule, i, j))
        {
          ...
          if (!isContact(contact_schedule, i_touchdown, j)) {
            ...
            Eigen::Vector3d body_pos_midstance = body_plan.block<1,3>(horizon_length_-1,0) +
              body_plan.block<1,3>(horizon_length_-1,6)*((i_liftoff + swing_duration +
              0.5*stance_duration)-(horizon_length_-1))*dt_;
            Eigen::Vector3d body_rpy_midstance = body_plan.block<1,3>(horizon_length_-1,3);
            quadKD_->worldToNominalHipFKWorldFrame(j, body_pos_midstance, body_rpy_midstance,
              foot_position_next);

            // Toe has 20cm radius so we need to shift the foot height from terrain
            foot_position_next.z() = terrain_grid_.atPosition("z", foot_position_next.head<2>(),
              grid_map::InterpolationMethods::INTER_NEAREST) + toe_radius;
        ...
        // For the first step, it might be duplicated in the same plan index so we need to 
        // refine the phase based on the time duration to next plan index
        double swing_idx = (i == 0) ? (i - i_liftoff + (dt_ - first_element_duration) / dt_)
          : (i - i_liftoff);
        ...
        {
          // Start from plan
          interp_phase = (2 * swing_idx >= swing_duration) ? (2 * swing_idx / swing_duration - 1) : (2 * swing_idx / swing_duration);
      ...
      if (isContact(contact_schedule, i, j))
      {
        ...
        {
          // If it's a new contact, keep the acceleration for swing ID interplation
          // It should not be used after contact since it will be GRF instead
