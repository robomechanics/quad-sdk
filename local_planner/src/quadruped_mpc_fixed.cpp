...
void QuadrupedMPC::update_friction(const double mu) {
  ...
  // A_con_dense_ = Eigen::MatrixXd::Zero(num_contact_constraints_,
  // num_control_vars_);
  ...
  for (int i = 0; i < 4; ++i) {
    C_step.block<num_constraints_per_leg_, 3>(num_constraints_per_leg_ * i,
                                              3 * i) = C_leg;
  }
  C_step(num_contact_constraints_per_step_ - 1, Nu_ - 1) = 1;

  for (int i = 0; i < N_; ++i) {  // iterate over horizon
    A_con_dense_.block<num_contact_constraints_per_step_, nu_>(
        num_contact_constraints_per_step_ * i, nu_ * i) = C_step;
  }
}

void QuadrupedMPC::update_weights(
    const std::vector<StateMatrix> &Q,
...
void QuadrupedMPC::update_dynamics(
  ...
  for (int i = 0; i < 4; ++i) {
    Bd.block<3, 3>(6, 3 * i) =
        Eigen::Matrix<double, 3, 3>::Identity() / m_ * dt_;
  }
  ...
  Eigen::Vector<double, N_> yaws =
      ref_traj.row(5);  // JN - SHOULD THIS BE ref_traj.col(5)???
  for (int i = 0; i < N_; ++i) {
    ...
    for (int j = 0; j < 4; ++j) {
      ...
      Bdi.block<3, 3>(9, 3 * j) =
          Iw_inv * foot_pos_hat * dt_;  // Eigen::Matrix3d::Zero();
    }
...
void QuadrupedMPC::update_contact(
    const std::vector<std::vector<bool>> contact_sequence, const double fmin,
    const double fmax) {
...
void QuadrupedMPC::get_cost_function(
    const StateTraj &ref_traj, Eigen::Vector<double, num_decision_vars_> &f) {
  // Construct fx vector
...
bool QuadrupedMPC::solve(const Eigen::VectorXd &initial_state,
                         const StateTraj &ref_traj, Eigen::MatrixXd & -) {
#ifdef PRINT_TIMING
