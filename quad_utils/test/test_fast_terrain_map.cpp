...
TEST(FastTerrainMapTest, testSpeedComparison) {
  ...
  // Create GridMap
  grid_map::GridMap grid_map_obj({"z","nx","ny","nz","z_filt","nx_filt","ny_filt","nz_filt"});
  ...
  //   grid_map_obj.getLength().x(), grid_map_obj.getLength().y(), x_size, y_size);
  ...
  double x,y,z;
  std::chrono::time_point<std::chrono::steady_clock> start_time, intermediate_time_1,
    intermediate_time_2, stop_time;
  ...
  for (int i = 0; i < N; i++) {
    ...
    z += grid_map_obj.atPosition("z",pos, grid_map::InterpolationMethods::INTER_NEAREST)/N;
    intermediate_time_2 = std::chrono::steady_clock::now();
    elapsed = std::chrono::duration_cast<std::chrono::duration<double>>(intermediate_time_2 - intermediate_time_1);
  ...
  elapsed = std::chrono::duration_cast<std::chrono::duration<double>>(stop_time - start_time);
  ...
  for (int i = 0; i < N; i++) {
    ...
    pos = {x,y};
    intermediate_time_1 = std::chrono::steady_clock::now();
    z += grid_map_obj.atPosition("z",pos, grid_map::InterpolationMethods::INTER_LINEAR)/N;
    intermediate_time_2 = std::chrono::steady_clock::now();
    elapsed = std::chrono::duration_cast<std::chrono::duration<double>>(intermediate_time_2 - intermediate_time_1);
  ...
  // printf("FastTerrainMap linear avg (sum of individual/iterations) = %.3fus\n", (double)1e6*ftm_lin_total_elapsed/timings.size());
  // printf("z avg = %.3f\n", (double)z);
