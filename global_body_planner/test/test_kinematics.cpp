...
TEST(GlobalBodyPlannerTest, testLeapAction) {
  ...
  for (int i = 0; i < N; i++) {
    ...
    bool is_valid_reverse =
        isValidStateActionPair(s_final_1, a, result, planner_config);
...
TEST(GlobalBodyPlannerTest, testUnitConnectAction) {
  ...
  RRTConnectClass rrt;
  int connect_result =
      rrt.attemptConnect(s1, s2, t_s, result, planner_config, FORWARD);
...
TEST(GlobalBodyPlannerTest, testUnitConnectActionSlope) {
  ...
  s1.pos << 0, 0, 0.3;
  s1.vel << vel, 0, 0;
  EXPECT_TRUE(abs(getDzFromState(s1, planner_config) - vel * grade) <
              kinematics_tol);
...
TEST(GlobalBodyPlannerTest, testUnitLeapActionSlope) {
  ...
  // Check that the action is valid and yields a state of the correct height wrt
  // the terrain
  bool is_valid_forward = isValidStateActionPair(s, a, result, planner_config);
  EXPECT_TRUE(is_valid_forward);
  EXPECT_TRUE(abs(result.s_new.pos[2] -
                  (result.s_new.pos[0] * grade + planner_config.H_NOM)) <
              kinematics_tol);
}
