...
def _inject_mujoco_elements(urdf_xml, profile, world_path, namespace,
    ...
    for idx, mjc_joint_name in profile['joint_map']:
        ...
        # Transmissions live INSIDE <ros2_control> (interleaved with their
        # joints) — that's what xacro's <xacro:mujoco_joint> macro produces
        # and what mujoco_ros2_control's transmission_loader scans. Putting
        # them as <robot> siblings parses cleanly but leaves the
        # ros2_control_joint -> MuJoCo actuator mapping empty, so commands
        # never reach the simulator and the robot stays in default pose.
