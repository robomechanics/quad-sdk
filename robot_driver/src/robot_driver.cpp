  ...
  for (int i = 0; i < num_feet_; ++i) {
    for (int j = 0; j < 3; ++j) {
      ...
      if (abs(cmd.torque_ff) >= torque_limits_[j]) {
        ...
        //     " "Nm", i, j, cmd.torque_ff, torque_limits_[j]);
      }
