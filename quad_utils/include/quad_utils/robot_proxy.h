...
//! A class for interfacing between dummy robot data and other quad-software
//! nodes and topics.
/*!
   RobotProxy contains the approximation for the real quad robot. Its purposes
   are purely for development. The class provides publishers for dummy robot
   data (encoder, imu, and mocap measurements) as well as a subscriber for
   control inputs.
*/
class RobotProxy {
 public:
 ...
 private:
   ...
   * @param[in] Joint state message contining desired position, velocity, and
   * torque for each joint
   */
  void grfArrayCallback(const quad_msgs::GRFArray::ConstPtr& msg);
