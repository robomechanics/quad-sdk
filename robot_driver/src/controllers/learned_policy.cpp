...
void LearnedPolicy::init(const std::vector<double>& stance_kp,
  ...
  loadONNXModel();
  // Build nominal stance in RobotLab Go2 order:
  ...
  nominal_stance_pose_ << abd, hip, knee, abd, hip, knee, abd, hip, knee, abd,
      hip, knee;
...
void LearnedPolicy::computeObservations(
  ...
  // Quad-SDK order is FL, RL, FR, RR. RobotLab Go2 was trained with
  // FR, FL, RR, RL, each as hip/thigh/calf, with preserve_order=true.
  joint_positions << raw_joint_positions(6), raw_joint_positions(7),
      raw_joint_positions(8), raw_joint_positions(0), raw_joint_positions(1),
      raw_joint_positions(2), raw_joint_positions(9), raw_joint_positions(10),
      raw_joint_positions(11), raw_joint_positions(3), raw_joint_positions(4),
      raw_joint_positions(5);

  joint_velocities << raw_joint_velocities(6), raw_joint_velocities(7),
      raw_joint_velocities(8), raw_joint_velocities(0), raw_joint_velocities(1),
      raw_joint_velocities(2), raw_joint_velocities(9),
      raw_joint_velocities(10), raw_joint_velocities(11),
      raw_joint_velocities(3), raw_joint_velocities(4),
      raw_joint_velocities(5);
...
void LearnedPolicy::runInference() {
  ...
  try {
    outs = session_->Run(Ort::RunOptions{nullptr}, in_names, &input_tensor,
                         1, out_names, 1);
  } catch (const Ort::Exception& e) {
  ...
  // RobotLab Go2 action order is FR, FL, RR, RL, each hip/thigh/calf.
  // Hip joints use a smaller action scale than thigh/calf joints.
  Eigen::VectorXd action_scale(12);
  action_scale << 0.125, 0.25, 0.25, 0.125, 0.25, 0.25, 0.125, 0.25, 0.25,
      0.125, 0.25, 0.25;
  Eigen::VectorXd robotlab_actions =
      raw_actions_.cwiseProduct(action_scale) + nominal_stance_pose_;

  // Map RobotLab FR,FL,RR,RL targets back to Quad-SDK FL,RL,FR,RR order.
  actions_ << robotlab_actions(3), robotlab_actions(4), robotlab_actions(5),
      robotlab_actions(9), robotlab_actions(10), robotlab_actions(11),
      robotlab_actions(0), robotlab_actions(1), robotlab_actions(2),
      robotlab_actions(6), robotlab_actions(7), robotlab_actions(8);
