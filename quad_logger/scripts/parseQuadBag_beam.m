function [data, varargout] = parseQuadBag_beam(varargin)
...
if isempty(stateGroundTruthData)
    ...
    stateGroundTruth.orientationQuat = cell2mat(cellfun(@(m) ...
        [m.Body.Pose.Orientation.X, m.Body.Pose.Orientation.Y, m.Body.Pose.Orientation.Z, m.Body.Pose.Orientation.W], stateGroundTruthData, 'UniformOutput', 0));
...
jointTorquesData = readMessages(select(bag,'Topic',['/', namespace, 'body_force/joint_torques']),'DataFormat','struct');
joint_torques = struct;
if isempty(jointTorquesData)
     warning('No data on joint torques state topic');
else
 
     joint_torques.time = cell2mat(cellfun(@(m) double(m.Header.Stamp.Sec) + double(m.Header.Stamp.Nsec)*1E-9, jointTorquesData, 'UniformOutput', 0));
 
     joint_torques.torques = cell2mat(cellfun(@(m) m.JointTorques', jointTorquesData, 'UniformOutput', 0));
...
% Adding joint torques

if ~isempty(fieldnames(joint_torques))
    joint_torques.time = joint_torques.time - startTime;
    data.jointTorques = joint_torques;
else
    data.jointTorques = [];
end
