...
TailPlanner::TailPlanner(ros::NodeHandle nh) {
  ...
  // Get rosparams
  std::string body_plan_topic, robot_state_topic, local_plan_topic,
      tail_plan_topic, cmd_vel_topic, grf_topic;
  ...
  quad_utils::loadROSParam(nh_, "/tail_controller/planner_update_rate",
                           update_rate_);
  ...
  // Setup pubs and subs
  tail_plan_pub_ =
      nh_.advertise<quad_msgs::LegCommandArray>(tail_plan_topic, 1);
  body_plan_sub_ =
      nh_.subscribe(body_plan_topic, 1, &TailPlanner::robotPlanCallback, this,
                    ros::TransportHints().tcpNoDelay(true));
  robot_state_sub_ =
      nh_.subscribe(robot_state_topic, 1, &TailPlanner::robotStateCallback,
                    this, ros::TransportHints().tcpNoDelay(true));
  local_plan_sub_ =
      nh_.subscribe(local_plan_topic, 1, &TailPlanner::localPlanCallback, this,
                    ros::TransportHints().tcpNoDelay(true));
  cmd_vel_sub_ = nh_.subscribe(cmd_vel_topic, 1, &TailPlanner::cmdVelCallback,
                               this, ros::TransportHints().tcpNoDelay(true));
...
void TailPlanner::robotStateCallback(
...
void TailPlanner::computeTailPlan() {
  if (last_local_plan_msg_ == NULL ||
      ...
      robot_state_msg_ == NULL || grf_msg_ == NULL)
    return;
  ...
  if ((t_now < last_local_plan_msg_->states.front().header.stamp.toSec()) ||
    ...
    ROS_ERROR_STREAM(
        "t_now: "
        << t_now << ","
        << "last_local_plan_msg_->states.front().header.stamp.toSec(): "
        << last_local_plan_msg_->states.front().header.stamp.toSec());
    return;
  }
  ...
  for (int i = 0; i < last_local_plan_msg_->states.size() - 1; i++) {
    if ((t_now >= last_local_plan_msg_->states[i].header.stamp.toSec()) &&
      ...
      first_element_duration_ =
          dt_ - (t_now - last_local_plan_msg_->states[i].header.stamp.toSec());
  ...
  if (use_twist_input_) {
    for (size_t i = 0; i < N_ + 1; i++) {
      ...
      if (idx > last_local_plan_msg_->plan_indices.size() - 1) {
        ROS_ERROR_STREAM(
            "Tail planner node couldn't find the correct ref leg plan at "
            "index: "
            << i);
      ...
      ref_body_plan_.row(i) = quad_utils::bodyStateMsgToEigen(
                                  last_local_plan_msg_->ref_states[idx].body)
                                  .transpose();
    }
  } else {
    for (size_t i = 0; i < N_ + 1; i++) {
  ...
  for (size_t i = 0; i < N_ + 1; i++) {
    ...
    if (idx > last_local_plan_msg_->plan_indices.size() - 1) {
      ROS_ERROR_STREAM(
          "Tail planner node couldn't find the correct ref leg plan at index: "
          << i);
      idx = last_local_plan_msg_->plan_indices.size() - 1;
    }
    ...
    if (i < N_) {
      grf_plan_.row(i) =
          quad_utils::grfArrayMsgToEigen(last_local_plan_msg_->grfs[idx])
              .transpose();
  ...
  if (!tail_planner_->computeDistributedTailPlan(
          ...
          grf_plan_, ref_ground_height_, first_element_duration_,
          same_plan_index_, tail_plan_, tail_torque_plan_))
    return;
