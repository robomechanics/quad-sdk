...
int main(int argc, char *argv[])
{
	...
	ros::init(argc, argv, "body_planner");
	ros::NodeHandle nh;
	...
	while (nh.ok())
	{
		...
		for (int i=0;i<interp_path.size(); ++i)
		{
			...
			pose_stamped.pose.position.x = interp_path[i][0];
			pose_stamped.pose.position.y = interp_path[i][1];
			pose_stamped.pose.position.z = interp_path[i][2];
