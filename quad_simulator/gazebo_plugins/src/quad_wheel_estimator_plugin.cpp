...
#undef LOG
#endif
#include "gazebo_plugins/quad_wheel_estimator_plugin.hpp"
...
bool load_wheel_joint_name(const rclcpp::Node::SharedPtr& node, int leg_index,
                           std::string& wheel_name) {
  const std::string leg_ns = "leg_" + std::to_string(leg_index);
  declare_if_missing<std::string>(node, leg_ns + ".joints.wheel.name", "");
  if (!node->get_parameter(leg_ns + ".joints.wheel.name", wheel_name) ||
      wheel_name.empty()) {
    return false;
  }
  return true;
...
bool load_robot_names(const rclcpp::Node::SharedPtr& node,
  ...
  for (int leg_index = 0; leg_index < 4; ++leg_index) {
    ...
    if (!load_leg_joint_names(node, leg_index, leg_joint_names[leg_index])) {
      return false;
    }
    if (!load_wheel_joint_name(node, leg_index, wheel_joint_names[leg_index])) {
      RCLCPP_FATAL(node->get_logger(),
                   "QuadWheelGroundTruthEstimator requires "
                   "'leg_%d.joints.wheel.name' but it was empty.",
                   leg_index);
      return false;
    }
  ...
  // First 12 entries: leg joints in [abad, hip, knee] order per leg.
  for (int leg_index = 0; leg_index < 4; ++leg_index) {
    joint_names.insert(joint_names.end(), leg_joint_names[leg_index].begin(),
                       leg_joint_names[leg_index].end());
  }
  // Last 4 entries: wheel joints in leg order.
  for (int leg_index = 0; leg_index < 4; ++leg_index) {
    joint_names.push_back(wheel_joint_names[leg_index]);
  }

  return !body_frame_name.empty() && joint_names.size() == 16;
}

}  // namespace

void QuadWheelGroundTruthEstimator::Configure(
  ...
  this->node_ = std::make_shared<rclcpp::Node>(
      "gz_quad_wheel_ground_truth_estimator", robot_ns, options);

  if (sdf->HasElement("updateRateHZ")) {
    this->update_rate_ = sdf->Get<double>("updateRateHZ");
    RCLCPP_INFO(this->node_->get_logger(),
                "QuadWheelGroundTruthEstimator initialized with rate %.1f Hz",
                this->update_rate_);
  } else {
    update_rate_ = 500.0;
  }
  ...
  if (!load_robot_names(this->node_, this->body_frame_name_,
    ...
    RCLCPP_FATAL(
        ...
        "Missing robot naming config for QuadWheelGroundTruthEstimator. "
        "Expected 'body.frame', 'leg_i.frames.(lower|toe)', "
        "'leg_i.joints.(abad|hip|knee).name', and "
        "'leg_i.joints.wheel.name'.");
    rclcpp::shutdown();
    return;
  }

  // Wait for /robot_description to arrive once so we don't start
  // publishing before robot_state_publisher is up. We do NOT instantiate
  // QuadKD2 here — that's a 12-joint legged-only kinematics library and
  // is not used by this estimator (feet come straight from Gazebo).
...
void QuadWheelGroundTruthEstimator::PostUpdate(
  ...
  if (now_s > 1e8) {
    RCLCPP_WARN_THROTTLE(this->node_->get_logger(), *this->node_->get_clock(),
                         1000,
                         "node_->now() returned wall clock time. Skipping.");
    return;
  }
  ...
  if (body_entity == gz::sim::kNullEntity) {
    RCLCPP_WARN_THROTTLE(this->node_->get_logger(), *this->node_->get_clock(),
                         2000, "Can't find body link in sdf.");
    return;
  }

  gz::sim::Link body_link(body_entity);
  std::array<gz::sim::Link, kNumLegs> toe_links;
  for (int i = 0; i < kNumLegs; ++i) {
    ...
    if (lower_entity == gz::sim::kNullEntity ||
      ...
      RCLCPP_WARN_THROTTLE(this->node_->get_logger(), *this->node_->get_clock(),
                           2000, "Can't find leg links in sdf.");
      return;
    }
  ...
  const gz::math::Vector3d v_w = lin_vel;
  const gz::math::Vector3d w_w = ang_vel;
  const gz::math::Vector3d v_b = q_bw * v_w;
  const gz::math::Vector3d w_b = q_bw * w_w;
  ...
  // 12 leg joints: same torque interpretation as GroundTruthEstimator.
  for (int i = 0; i < kNumLegJoints; ++i) {
    ...
    if (joint_entity) {
      ...
      if (pos_opt && !pos_opt->empty()) pos = (*pos_opt)[0];
      if (vel_opt && !vel_opt->empty()) vel = (*vel_opt)[0];
      if (wrench_opt) {
        ...
        switch (i % 3) {
          case 0:  // Abad (axis x)
            torque = torque_msg.x();
            break;
          case 1:  // Hip (axis y, sign negated to match GroundTruthEstimator)
            torque = -torque_msg.y();
            break;
          case 2:  // Knee (axis y)
            torque = torque_msg.y();
  ...
  // 4 wheel joints (continuous, axis y per the URDF). Torque is read off
  // the y-component of the transmitted wrench.
  for (int w = 0; w < kNumWheels; ++w) {
    const int idx = kNumLegJoints + w;
    auto joint_entity = this->model_.JointByName(ecm, state.joints.name[idx]);
    double pos = 0.0, vel = 0.0, torque = 0.0;
    if (joint_entity) {
      gz::sim::Joint joint(joint_entity);
      auto pos_opt = joint.Position(ecm);
  ...
  // We skip quad_utils::fkRobotState / updateDynamics here. Those
  // helpers wrap QuadKD2, whose internal Pinocchio nv_ is locked at
  // construction from the URDF — and the go2w URDF carries 16 actuated
  // joints + a 6-DoF floating base, so nv_ = 22. fkRobotState writes
  // into a hard-coded Eigen::MatrixXd::Zero(12, 18) Jacobian via
  // jacobian.block(*, nv_-6, 3, 3) etc., which overruns the 18-col
  // buffer whenever nv_ > 18 and corrupts the heap (observed as a
  // double-free on the next allocation). Feeding a leg-only JointState
  // doesn't fix this — nv_ is already 22.
  //
  // What fkRobotState would have set on `state.feet`:
  //   feet.header                (joint_state.header)
  //   feet.feet[i].position      (kinematic FK — overwritten below by
  //                               Gazebo's exact toe link pose)
  //   feet.feet[i].velocity      (J*qdot — overwritten below by
  //                               Gazebo's exact toe link velocity)
  //   feet.feet[i].header        (= feet.header)
  // We populate the headers explicitly here and then write the
  // positions / velocities from Gazebo, which is exact and what the
  // base GroundTruthEstimator ultimately uses anyway.
  state.feet.feet.resize(kNumLegs);
  state.feet.header = state.joints.header;

  for (int i = 0; i < kNumLegs; ++i) {
    state.feet.feet[i].header = state.feet.header;
  ...
  const gz::math::Vector3d p_body_w = pose.Pos();
  for (int i = 0; i < kNumLegs; ++i) {
    auto toe_pose_opt = toe_links[i].WorldPose(ecm);
    auto toe_vel_opt = toe_links[i].WorldLinearVelocity(ecm);
...
GZ_ADD_PLUGIN(gz_plugins::QuadWheelGroundTruthEstimator, gz::sim::System,
              gz::sim::ISystemConfigure, gz::sim::ISystemPostUpdate)

GZ_ADD_PLUGIN_ALIAS(gz_plugins::QuadWheelGroundTruthEstimator,
