...
TEST(TestUseCase, quadVariable) {
  ...
  double Rfy = 1e-6;
  double Rfz = 1e-6;
  Ru.diagonal() << Rfx, Rfy, Rfz, Rfx, Rfy, Rfz, Rfx, Rfy, Rfz, Rfx, Rfy, Rfz,
      0;

  // State bounds (fixed for a given solve)
  Eigen::VectorXd state_lo(Nx);
  state_lo << -100, -100, 0.15, -M_PI / 4, -M_PI / 4, -4 * M_PI, -5, -5, -2,
      -M_PI, -M_PI, -M_PI;
  Eigen::VectorXd state_hi(Nx);
  state_hi << 100, 100, 0.4, M_PI / 4, M_PI / 4, 4 * M_PI, 5, 5, 2, M_PI, M_PI,
      M_PI;
  ...
  foot_position << body_l / 2, body_w / 2, -0.29, -body_l / 2, body_w / 2,
      -0.29, body_l / 2, -body_w / 2, -0.29, -body_l / 2, -body_w / 2, -0.29;
  ...
  // std::cout << "control_traj" << std::endl << control_traj << std::endl <<
  // std::endl;; std::cout << "initial_state" << std::endl << initial_state <<
  // std::endl << std::endl;; std::cout << "ref_traj" << std::endl << ref_traj
  // << std::endl << std::endl;; std::cout << "foot_positions" << std::endl <<
  // foot_positions << std::endl << std::endl;;
