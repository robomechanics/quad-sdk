...
TerrainMapPublisher::TerrainMapPublisher(ros::NodeHandle nh)
  ...
  nh.param<std::string>("terrain_map_publisher/map_data_source",
                        map_data_source_, "internal");
  nh.param<std::string>("terrain_map_publisher/terrain_type", terrain_type_,
                        "slope");
  // Setup pubs and subs
  terrain_map_pub_ =
      nh_.advertise<grid_map_msgs::GridMap>(terrain_map_topic, 1);

  // Add image subscriber if data source requests an image
  if (map_data_source_.compare("image") == 0) {
    nh_.param<std::string>("topics/image", image_topic,
    ...
    image_sub_ =
        nh_.subscribe(image_topic, 1, &TerrainMapPublisher::loadMapFromImage,
