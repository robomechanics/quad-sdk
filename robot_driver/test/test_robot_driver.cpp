...
TEST(LegController, testConstructorRobotDriver) {
  ...
  std::string rd_pkg_share = ament_index_cpp::get_package_share_directory("robot_driver");
  std::string utils_pkg_share = ament_index_cpp::get_package_share_directory("quad_utils");
  std::string robot_driver_param_file = rd_pkg_share + "/config/robot_driver.yaml";
  std::string robot_driver_topics_file = rd_pkg_share + "/config/robot_driver_topics.yaml";
  std::string robot_specific_param_file = utils_pkg_share + "/config/spirit.yaml";
  
  const std::string xacro_path =
    ament_index_cpp::get_package_share_directory("spirit_description") + "/models/spirit/urdf/spirit.urdf.xacro";
  const std::string urdf_string = runXacro(xacro_path);

  rclcpp::NodeOptions options;
  options.arguments({"--ros-args", 
    "--params-file", robot_driver_param_file,
    "--params-file", robot_driver_topics_file,
    "--params-file", robot_specific_param_file});
  options.append_parameter_override("use_sim_time", false);
  options.append_parameter_override("robot_description", urdf_string);

  auto node = std::make_shared<rclcpp::Node>("robot_driver", "robot_1", options);
