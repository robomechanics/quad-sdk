...
void GlobalBodyPlanner::goalStateCallback(const geometry_msgs::PointStamped::ConstPtr& msg) {
  ...
  // If the old plan has been executed, allow full replanning, otherwise immediately update plan
  if (current_plan_.getDuration() <= (ros::Time::now() - current_plan_.getPublishedTimestamp()).toSec()) {
    triggerReset();
  }
}
...
bool GlobalBodyPlanner::callPlanner() {
  ...
  for (int i = 0; i<num_calls_; ++i)
  {
    ...
    int plan_status = rrt_connect_obj.runRRTConnect(planner_config_, start_state, goal_state,
...
void GlobalBodyPlanner::getInitialPlan() {
  ...
  while (ros::ok() && ((ros::Time::now() - start_time) < ros::Duration(reset_publish_delay_))) {
...
void GlobalBodyPlanner::publishCurrentPlan() {
  ...
  if (publish_after_reset_delay_ || newest_plan_ == current_plan_) {
    ...
    // Declare the messages for interpolated body plan and discrete states, initialize their headers
