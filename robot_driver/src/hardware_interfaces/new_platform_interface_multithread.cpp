...
NewPlatformInterfaceMultithread::NewPlatformInterfaceMultithread() {}
...
void printMotorIds(vector<int> motorIds, int canBusNum) {
	int numMotors = motorIds.size();
    ROS_INFO("CAN %d. Motor Ids: {");
...
void printMotorStates(vector<int> motorIds, std::map<int, motor_driver::motorState> motorStates) {
	int numMotors = motorIds.size();
	for (int i = 0; i < numMotors; i++) {
...
void NewPlatformInterfaceMultithread::loadInterface(int argc, char** argv) {
    ...
    ROS_INFO_STREAM("CAN 0 Positions: " << printMotorStates(motor_ids_0, motorStates_0));
    ROS_INFO_STREAM("CAN 1 Positions: " << printMotorStates(motor_ids_1, motorStates_1));

}

void NewPlatformInterfaceMultithread::unloadInterface() {
...
bool NewPlatformInterfaceMultithread::send(
    ...
    std::thread motorDriverThread_0;
    std::thread motorDriverThread_1;
    ...
    for (int i = 0; i < 4; ++i) {  // For each leg
        ...
        case 2: //FR
            ...
            ROS_INFO("Pos_Set: %f. Index: %i",pos,idx_offset+j);
        ...
        case 3: //BR
            ...
            //Initialize thread for motor driver execution
            motorDriverThread_1(sendMotorCommand_1, commandStruct_1);
            }
            break;
    ...
    if (motorDriverThread_0.joinable()) {
        motorDriverThread_0.join();
    }
    if (motorDriverThread_1.joinable()) {
        motorDriverThread_1.join();
    }
    ...
    bool NewPlatformInterfaceMultithread::recv(sensor_msgs::JointState& joint_state_msg,
    ...
    for (int j = 0; j < joint_names_1.size(); j++) {
    ...
    ROS_INFO("Motor %s Position Received: %f", joint_state_msg.name[1], joint_state_msg.position[1]);
