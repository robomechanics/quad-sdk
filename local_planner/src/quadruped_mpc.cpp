...
QuadrupedMPC::QuadrupedMPC() {
  ...
  num_constraints_ = num_state_vars_ + num_dyn_constraints_ +
                     num_contact_constraints_;  // + num_control_vars_;
...
void QuadrupedMPC::update_friction(const double mu) {
  ...
  A_con_dense_ =
      Eigen::MatrixXd::Zero(num_contact_constraints_, num_control_vars_);

  // Friction cone for one leg
  Eigen::MatrixXd C_leg(num_constraints_per_leg_, 3);
  C_leg << 1, 0, -mu, 1, 0, mu, 0, 1, -mu, 0, 1, mu, 0, 0, 1;

  // Full friciton cone for one tstep
  Eigen::MatrixXd C_step =
      Eigen::MatrixXd::Zero(num_contact_constraints_per_step_, nu_);
  for (int i = 0; i < 4; ++i) {
    C_step.block(num_constraints_per_leg_ * i, 3 * i, num_constraints_per_leg_,
                 3) = C_leg;
  }
  C_step(num_contact_constraints_per_step_ - 1, nu_ - 1) = 1;

  for (int i = 0; i < N_; ++i) {  // iterate over horizon
    A_con_dense_.block(num_contact_constraints_per_step_ * i, nu_ * i,
                       num_contact_constraints_per_step_, nu_) = C_step;
  }
}
...
void QuadrupedMPC::update_dynamics_hip_projected_feet(
  ...
  for (int i = 0; i < N_; ++i) {
    for (int j = 0; j < 4; ++j) {
      ...
      this->quadKD_->bodyToFootFKBodyFrame(j, nominal_joint_state,
                                           toe_body_pos);
...
void QuadrupedMPC::update_dynamics(
    const Eigen::MatrixXd &ref_traj,
  ...
  A_dyn_dense_ =
      Eigen::MatrixXd::Zero(num_dyn_constraints_, num_decision_vars_);
  ...
  for (int i = 0; i < N_; ++i) {
    ...
    for (int i = 0; i < num_feet_; ++i) {
      Bdi.block(6, 3 * i, 3, 3) =
          Eigen::MatrixXd::Identity(3, 3) / m_ * dt_vec_[i];
    }

    // Update our net moment due to foot positions
    for (int j = 0; j < num_feet_; ++j) {
      Eigen::Matrix3d foot_pos_hat;
      foot_pos_hat << 0, -foot_positions(i, 3 * j + 2),
          foot_positions(i, 3 * j + 1), foot_positions(i, 3 * j + 2), 0,
          -foot_positions(i, 3 * j + 0), -foot_positions(i, 3 * j + 1),
          foot_positions(i, 3 * j + 0), 0;
      Bdi.block(9, 3 * j, 3, 3) =
          Iw_inv * foot_pos_hat * dt_vec_[i];  // Eigen::Matrix3d::Zero();
    }

    Bdi(8, 12) = -g_ * dt_vec_[i];  // gravity acts downwards here

    A_dyn_dense_.block(nx_ * i, nx_ * i, nx_, nx_) = Adi;
    A_dyn_dense_.block(nx_ * i, nx_ * (i + 1), nx_, nx_) =
        -Eigen::MatrixXd::Identity(nx_, nx_);
...
void QuadrupedMPC::update_contact(
    const std::vector<std::vector<bool>> contact_schedule, const double fmin,
    const double fmax) {
  ...
  for (int i = 0; i < N_; ++i) {           // iterate over horizon
    for (int j = 0; j < num_feet_; ++j) {  // iterate over legs
      int row_start =
          num_contact_constraints_per_step_ * i + num_constraints_per_leg_ * j;
    ...
    b_contact_lo_(num_contact_constraints_per_step_ * i +
                  num_contact_constraints_per_step_ - 1) = 1.0;
    b_contact_hi_(num_contact_constraints_per_step_ * i +
                  num_contact_constraints_per_step_ - 1) = 1.0;
  }
}
...
void QuadrupedMPC::update_control_bounds(const double f_min,
                                         const double f_max) {
  f_min_ = f_min;
  f_max_ = f_max;
}
...
void QuadrupedMPC::get_cost_function(const Eigen::MatrixXd &ref_traj,
                                     Eigen::VectorXd &f) {
  // Construct fx vector (reshape is only column major so columns need to be
  // state vectors)
...
void QuadrupedMPC::get_output(const Eigen::MatrixXd &x_out,
  ...
  // Get final cost
  f_val = (0.5 * x_out.transpose() * H_ * x_out)(0, 0) +
          (f_.transpose() * x_out)(0, 0);
}
...
bool QuadrupedMPC::solve(const Eigen::VectorXd &initial_state,
  ...
  Eigen::MatrixXd A =
      Eigen::MatrixXd::Zero(num_constraints_, num_decision_vars_);

  A.block(0, 0, num_dyn_constraints_, num_decision_vars_) = A_dyn_dense_;
  A.block(num_dyn_constraints_, 0, num_state_vars_, num_state_vars_) =
      Eigen::MatrixXd::Identity(num_state_vars_, num_state_vars_);
  A.block(num_dyn_constraints_ + num_state_vars_, num_state_vars_,
          num_contact_constraints_, num_control_vars_) = A_con_dense_;
  ...
  std::cout << "QuadrupedMPC::solve completed in " << time_span.count() * 1000.0
...
bool QuadrupedMPC::computePlan(
  ...
  // std::cout << "initial_state" << std::endl << initial_state << std::endl <<
  // std::endl; std::cout << "ref_traj" << std::endl << ref_traj << std::endl <<
  // std::endl; std::cout << "foot_positions" << std::endl << foot_positions <<
  // std::endl << std::endl; std::cout << "state_traj" << std::endl <<
  // state_traj << std::endl << std::endl; std::cout << "control_traj" <<
  // std::endl << control_traj << std::endl << std::endl; throw
  // std::runtime_error("STOP AND CHECK");
