...
namespace mesh_to_grid_map
{
          ...
          world_name_(kDefaultWorldName)
    {
        ...
        std::string package_path = ament_index_cpp::get_package_share_directory("gazebo_scripts");
        ...
        if (base_name.size() >= extension.size() &&
            base_name.compare(base_name.size() - extension.size(), extension.size(), extension) == 0)
        {
    ...
    void MeshToGridMapConverter::subscribeToTopics() // UPDATED
    {
        ...
        mesh_sub_ = node_->create_subscription<pcl_msgs::msg::PolygonMesh>("mesh", rclcpp::QoS(10), 
            std::bind(&MeshToGridMapConverter::meshCallback, this, std::placeholders::_1));
    }
    ...
    void MeshToGridMapConverter::advertiseTopics() // UPDATED 
    {
        ...
        grid_map_pub_ = node_->create_publisher<grid_map_msgs::msg::GridMap>("terrain_map_raw", qos_profile);
    ...
    void MeshToGridMapConverter::advertiseServices() // UPDATED
    {
        save_grid_map_srv_ = node_->create_service<grid_map_msgs::srv::ProcessFile>(
            ...
            std::bind(&MeshToGridMapConverter::saveGridMapService, this, std::placeholders::_1, std::placeholders::_2));
      
        load_map_service_server_ = node_->create_service<grid_map_msgs::srv::ProcessFile>(
            "load_mesh_from_file",
            std::bind(&MeshToGridMapConverter::loadMeshService, this, std::placeholders::_1, std::placeholders::_2));
        ...
        const uint64_t& time_stamp_nano_seconds)
    {
        ...
        //             std::cout << "Corner " << i << " at index (" << index(0) << ", " << index(1)
        //                       << ") has z = " << z << std::endl;
        //         } catch (const std::exception& e) {
        //             std::cerr << "Error accessing z at corner " << i << ": " << e.what() << std::endl;
        //         }
        //     }
        // }
        ...
        if (verbose_)
        {
            // RCLCPP_INFO_STREAM("Number of polygons: " << polygon_mesh.polygons.size());
                                             ...
                                             const std::string& topic_name)         //UPDATED
    {
        ...
        {
            RCLCPP_ERROR(node_->get_logger(), "No rosbag filepath specified where to save grid map.");
            return false;
        }
        return true;
    }
        ...
        std::shared_ptr<grid_map_msgs::srv::ProcessFile::Response> response)
    {
        ...
        {
            RCLCPP_WARN(node_->get_logger(), "Field 'topic_name' in service request will not be used.");
        }
        response->success =
        ...
        const std::string &path_to_mesh_to_load)
    {
        ...
        if (!mesh_converted)
        {
            RCLCPP_ERROR(node_->get_logger(), "It was not possible to convert loaded mesh to grid_map object.");
            return false;
        }

        if (verbose_)
        {
            RCLCPP_INFO(node_->get_logger(), "Loaded the mesh from file: %s. Its frame_id is set to '%s'",
